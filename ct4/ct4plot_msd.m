function D = ct4plot_msd(msd_compil, timelag, clr, fit_lin, fit_ano, text_offset, do_log)
% function D = ct4plot_msd(msd_compil, timelag, clr, fit_lin, fit_ano, text_offset, do_log)
...
if nargin<2 % if timelag == -1...
    use_units = 0;% % %     unit_strings = {'(img' 'pxlÂ²'};
    timelag = 1;
else
    use_units = 1;
end
...
if ~isempty(msd_compil)
    ...
    if ~do_log
        plot(tt, r2(1:nmax, 2), 'color', clr), hold on
        plot(tt, r2(1:nmax, 2)+r2(1:nmax, 3), 'color', clr, 'linestyle', ':')
        plot(tt, r2(1:nmax, 2)-r2(1:nmax, 3), 'color', clr, 'linestyle', ':')
    else
        loglog(tt, r2(1:nmax, 2), 'color', clr), hold on
        loglog(tt, r2(1:nmax, 2).*r2(1:nmax, 3), 'color', clr, 'linestyle', ':')
        loglog(tt, r2(1:nmax, 2)./r2(1:nmax, 3), 'color', clr, 'linestyle', ':')
    % set(gca, 'XScale', 'log', 'YScale', 'log'), end % 8/7/2021
    end
    
    if use_units, xlabel ('t (s)'), ylabel('MSD (µm²)')
    else, xlabel ('t (img)'), ylabel('MSD (pxl²)')
    end
    
    %% calcul D et RD, AS 8/3/5
    D = calculDinst(r2); % = [n=1 D dD o]
    ...
    if fit_ano
        ...
        if use_units
            Da = Da/timelag;
            s3 = sprintf('D_{anom}=%4.2gµm²/s', Da);
        else
            s3 = sprintf('D_{anom}=%4.2gpxl²/img', Da);
        end
        s4 = sprintf('\\gamma=%4.2g', gamma);
        
        if Da > 0, plot(ttf, 4*Da*ttf.^gamma*timelag^(1-gamma) + offset_a, 'color', clr, 'linestyle', '--'), end % fit anomal 'm'??
    else, s3 = ''; s4 = '';
    end
