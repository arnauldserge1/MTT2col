function carto_movie_2colors3(file, file_out)
% trace les trajs (xy) image apres image (t)
% sans ombre, bien + rapide
...
dir_out = '/Users/serge/Documents/labo/articles/JAM nanodyn/videos/videos with bar and time/';
file_out = [dir_out file_out(1:end-4) '_xyt.avi'];
...
% writerObj.Height = 512; % or more for super res? % writerObj.VideoCompressionMethod = 'H.264'; % as recommended by NatMet
open(writerObj);
disp('frame            ')
...
for t = 1:Tmax-1
    ...
    for nt = 1:length(ok) % Ntrc ok
        logD = logDG(t, ok(nt)); % coding for D @ current step (or fast/diff/lin/conf??)
    ...
    for nc = 1:length(ok) % Ncoloc @ t
        h(nc) = plot(tab_jR(t, ok(nc)), tab_iR(t, ok(nc)), 'wo');%, 'markersize', 16)
    end
...
fprintf([repmat('\b', 1, 11) 'finished, with last one: %5i/%5i\r'], Tmax, Tmax)
writeVideo(writerObj, frame);
close(writerObj);
%%%