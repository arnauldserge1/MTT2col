...
if ~isempty(strfind(cd,'HTHLAB')) || strfind(filename,'_filt'), Imax = 255; end

%% boucle / file
for i = 1:length(files)
    file = files(i).name;
    disp('loading data...')
%     if strcmp(viewDV,'_right') && isempty(dir(['pknico\', file, viewDV, '.pk']))%         viewDV='_rigth'; % bug aurtografic!%     end
    if strcmp(codage, 'int'), [trcdata, pkdata] = detect_reconnex_to_trc(file);
    else trcdata = detect_reconnex_to_trc(file);
    end
...
%% calcul du MSD (si utile, selon codage)
    if strcmp(codage, 'speed')
        %r2 = trc(:,3).^2+trc(:,4).^2; max_dr = sqrt(max(r2));
        NewFullOutput = 1; print_out = 1; first_step_only = 1; % do_compil = 0; def
        [~, fmsddata] = msd(trcdata, NewFullOutput, print_out, first_step_only);
        max_dr = sqrt(max(fmsddata{1}));
    end
    
    ntrc = trcdata(end,1); % sum(diff(trcdata(:,1))>0)+1;
...
%         if ~isempty(Ni) % bug si traj fantome (si supprimée par seuils...)
...
%                 indi = inddata(itrc,2:end);  indi = indi(indi>0); % supprime les zeros
                pki = pkdata(trcdata(:,1)==itrc,:); % indi,:);
...
%%  *** calcul index Lconf de Saxton ***
            if (strcmp(codage, 'conf') || strncmp(codage, 'var', 3)) && length(n_trci)>Sm
                Lconf = probaconf(trci, graph, timing, codage);
            end
