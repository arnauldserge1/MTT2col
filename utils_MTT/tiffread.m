...
if strcmp(filename(end-2:end),'tif') && nargout>1 %AS 24/9/8
    stack_cnt = length(imfinfo(filename));  %%% ASSEZ LONG.................
end
...
%                     defined in the TIFF file'); AS 24/9/8 pour images lsm
                end
            case 269
...
%                 end
            ...
            case {50838, 50839} % AS 19/12/8 (images Zeiss..)
            case 266 % AS 2020 (spinning, Alexandra)
            otherwise
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        Nimg = img_last - img_first + 1;
        stack = zeros(IMG.height * Nimg, IMG.width); % AS 19/12/8
        ...
        for ii = img_first:img_last
            ...
            if exist('waitbar_handle', 'var')
                ...
                waitbar( img_read/Nimg, waitbar_handle);                 
            end
            ...
            img_read = img_read + 1;
            %stack( img_read ) = IMG;
            lines = (img_read-1)*IMG.height + 1:IMG.height; % AS 19/12/8
            if isequal(size(stack (lines,:)), size(double(IMG.data))) % AS 3/4/9
                stack (lines,:) = double(IMG.data);
            end
        end
        break;
