function  [Msdout,FullMsdMatrix,MSDcompil] = ...
    msd (Trc, NewFullOutput, print_out, first_step_only, do_compil, MTT_format, compute_std, use_sem, do_log)
...
%                deviation for each particle [n t msd sd] % or sem???
...
%          MSDcompil = [n msd_n std(msd_n)], % or sem = std(msd_n)/sqrt(n) 
%           with n=length(msd_n), number of msd elements for each step #n
%           if do_log, MSDcompil in logmean, logstd
%
...
if nargin<7, compute_std = 1; end
if nargin<8, use_sem = 1; end % 8/7/2021
if nargin<8, do_log = 0; end % 8/7/2021
...
for Ipart=1:MaxPart % for each trace
    if mod(Ipart, 20)==0 && print_out % if mod(Ipart,round(MaxPart/10))==0 && print_out
    ...
    if ~isempty(iTrc) % if there are points in this trace
        ...
        if DoFullMSD || do_compil % someone really wants ALL the data (ou au moins compil..)
            if first_step_only && ~do_compil % AS 3/5/2006 -> 24/11/6 add do_compil (qui n�cessite full msd, transitoirement)
                Nmax = 1;
            else
            ...
            if NewFullOutput
                ...
                for i=1:Nmax
                    FullMsdMatrix{i} = [FullMsdMatrix{i},H2(i,1:(indeX(i)-1))]; %#ok
                end
            else
                if Nstep<Nmax   % bugger, matrix too small
                    FullMsdMatrix = [FullMsdMatrix, zeros(FMMlen,Mlag-Nmax)]; %#ok
                    Nstep = Nmax;
                end
                for i=1:Nmax
                    FullMsdMatrix((FMMlen)+(1:(indeX(i)-1)),i) = H2(i,1:(indeX(i)-1))'; %#ok
        ...
        for il=1:Mlag
            ...
            if noZ>0
                ...
                Lag(ii)  = il;
                Msd(ii)  = mean(H1);
                if compute_std % AS 2014
                    if noZ>1
                        if use_sem
                            dMsd(ii) = std(H1)/sqrt(noZ);
                        else
                            dMsd(ii) = std(H1); % 8/7/2021
                        end
                    else
                        dMsd(ii) = Msd(length(Msd)); % 0 ???
                    end
                end
                ii=ii+1;
            end
...
if do_compil && ~isempty(FullMsdMatrix)
    ...
    for n=1:length(FullMsdMatrix) % =ntrc
        ...
        if do_log % 8/7/2021
            log_val = log(val);
            if use_sem % 8/7/2021
                mean_geo = exp(mean(log_val));
                sem_geo = std(log_val)/sqrt(nval); % 13/7/21!! (n-1??)
                MSDcompil(n,:) = [n, mean_geo, sem_geo];
            else
                MSDcompil(n,:) = [n, exp(mean(log_val)), exp(std(log_val))];
            end
        else
            if use_sem % 8/7/2021
                MSDcompil(n,:) = [n, mean(val), std(val)/sqrt(nval)];
            else
    ...
    if first_step_only 
        FullMsdMatrix = FullMsdMatrix{1}; % �co. m�moire vive
    end
end
